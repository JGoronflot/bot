...
bot.login("NjQwODQ1NjQ3Mzg1NDYwNzM3.XkFK0w.7UeWDo4S8tl5sQsMmmkpuAxnhZw");
    ...
    if(user != bot.user){
        connection.query('SELECT * FROM classes WHERE react="'+reaction.emoji.name.replace("_Femme", "").replace("_Homme", "")+'"', function (error, results, fields) {
            if (error) throw error;
              var role = reaction.message.guild.roles.find(role => role.id === results[0].roleid);
              var role2 = reaction.message.guild.roles.find(role => role.id === "676354733430145024");
              var role3 = reaction.message.guild.roles.find(role => role.id === "676393968598122517");
              var member = reaction.message.guild.members.find(member => member.id === user.id);
              member.addRole(role).catch(err = console.error);
              member.addRole(role2).catch(err = console.error);
              member.removeRole(role3).catch(err = console.error);
  ...
  if(reaction.message.channel.id == "656118230359867392"){
    if(user != bot.user){
        connection.query('SELECT * FROM races WHERE react="'+reaction.emoji.name.replace("_Femme", "").replace("_Homme", "")+'"', function (error, results, fields) {
            if (error) throw error;
              var role = reaction.message.guild.roles.find(role => role.id === results[0].roleid);
              var role2 = reaction.message.guild.roles.find(role => role.id === "676393968598122517");
              var member = reaction.message.guild.members.find(member => member.id === user.id);
              member.addRole(role).catch(err = console.error);
              member.addRole(role2).catch(err = console.error);
              var roleToRemove = reaction.message.guild.roles.find(role => role.id === "676354733430145024");
              member.removeRole(roleToRemove).catch(err = console.error);
            });
    }
  }
})
    ...
    if(user != bot.user){
        connection.query('SELECT * FROM classes WHERE react="'+reaction.emoji.name.replace("_Femme", "").replace("_Homme", "")+'"', function (error, results, fields) {
            if (error) throw error;
                var role = reaction.message.guild.roles.find(role => role.id === results[0].roleid);
                var member = reaction.message.guild.members.find(member => member.id === user.id);
                member.removeRole(role).catch(err = console.error);
            });
    }
  }
  if(reaction.message.channel.id == "656118230359867392"){
