...
module.exports = class Class extends Command {
    static match (client){
        const channel = client.channels.get('656118120540536853');
        ...
        if (error) throw error;
            for(var i = 0; i < results.length;i++){
                if(results[i].enable == 1){
                    var emoji_homme = client.emojis.find(emoji => emoji.name == results[i].react+"_Homme");
                    var emoji_femme = client.emojis.find(emoji => emoji.name == results[i].react+"_Femme");
            ...
            channel.send("Choix de la classe ?").then(async msg => {
                ...
                if (error) throw error;
                    for(var i = 0; i < results.length;i++){
                        if(results[i].enable == 1){
                            var emojireact_homme = client.emojis.find(emoji => emoji.name == results[i].react+"_Homme");
                            msg.react(emojireact_homme.id);
                            var emojireact_femme = client.emojis.find(emoji => emoji.name == results[i].react+"_Femme");
